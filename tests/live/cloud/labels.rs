...
async fn returns_a_well_formed_page_of_site_wide_labels() {
    let page = cloud()
        ...
        .send()
        .await
        .expect("the site lists its labels");

    assert!(page.values.is_some(), "a page carries its values");
    assert!(
        page.total.is_some(),
        "a page carries how many labels there are in total"
    );
...
async fn eventually_lists_the_label_just_put_on_an_issue() {
    ...
    assert!(
        ...
        "the label an issue carries is a label of the site"
    );
...
async fn honours_max_results_and_start_at() {
    ...
    assert!(
        first.values.as_deref().unwrap_or_default().len() <= 2,
        "the limit is honoured, not merely echoed"
    );
    ...
    if first.total.unwrap_or_default() > 2 {
        ...
        assert_ne!(
            ...
            "an offset moves the window rather than repeating it"
        );
    }
}
...
async fn reports_is_last_truthfully_at_the_end_of_the_listing() {
    let all = cloud()
        ...
        .send()
        .await
        .expect("the whole listing is readable");
