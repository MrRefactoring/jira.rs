...
fn parses_as_a_date(value: &str) -> bool {
    ...
    let Some(Ok(year)) = fields.next().map(str::parse::<u32>) else { return false };
    let Some(Ok(month)) = fields.next().map(str::parse::<u32>) else { return false };
    let Some(Ok(day)) = fields.next().map(str::parse::<u32>) else { return false };
