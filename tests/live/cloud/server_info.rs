...
async fn returns_the_site_identity_typed_as_server_information_declares() {
    let info = cloud()
        ...
        .send()
        .await
        .expect("the site names itself");

    assert!(
        info.base_url.as_deref().is_some_and(|url| url.starts_with("https://")),
        "{:?}",
        info.base_url
    );
    assert_eq!(
        info.deployment_type.as_deref(),
        Some("Cloud"),
        "these suites run against Cloud, not Data Center"
    );
    assert!(
        info.version.as_deref().is_some_and(|version| !version.is_empty()),
        "a site reports its version"
    );
    assert!(
        !info.version_numbers.unwrap_or_default().is_empty(),
        "the version is also given field by field"
    );
}
...
async fn reports_a_build_date_and_server_time_that_parse_as_real_dates() {
    ...
    assert!(
        parses_as_a_date(&server_time),
        "the server time is a date: {server_time}"
    );
}
