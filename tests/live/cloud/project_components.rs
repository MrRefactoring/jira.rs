...
async fn walks_a_component_through_its_lifecycle() {
    ...
    assert!(
        component_id.chars().all(|c| c.is_ascii_digit()),
        "an id is digits: {component_id}"
    );
    assert_eq!(created.name.as_deref(), Some(name.as_str()));
    assert_eq!(created.project.as_deref(), Some(TEST_PROJECT_KEY));
    assert!(
        created.self_.is_some_and(|link| link.starts_with("https://")),
        "a component carries a self link"
    );
    ...
    let read = cloud()
        ...
        .send()
        .await
        .expect("the component reads back by id");
    ...
    assert!(
        all.iter()
            .any(|component| component.id.as_deref() == Some(component_id.as_str())),
    ...
    cloud()
        ...
        .edit_issue(
            ...
            IssueUpdateDetails {
                fields: Some(
                    [("components".to_owned(), json!([{ "id": component_id }]))]
                        .into_iter()
                        .collect(),
                ),
    ...
    cloud()
        ...
        .send()
        .await
        .expect("the component can be deleted");
    ...
    assert!(
        components.is_empty(),
        "deleting the component leaves the issue intact and unattached"
    );
