...
async fn reads_the_account_the_credentials_belong_to() {
    ...
    assert!(
        myself.account_id.is_some_and(|id| !id.is_empty()),
        "an authenticated user has an account id"
    );
    assert!(
...
async fn creates_reads_and_deletes_an_issue() {
    ...
    assert!(
        after.is_err_and(|error| error.is_not_found()),
        "the issue is gone once cleanup has run"
    );
}
