...
async fn walks_a_watcher_through_its_lifecycle() {
    ...
    let watching = cloud().issue_watchers().get_issue_watchers(&issue.key).send().await.expect("the watchers read back");
    ...
    assert_eq!(
        bulk.issues_is_watching.as_ref().and_then(|watching| watching.get(&issue.id)).and_then(
